...
void SplitWords(const std::string& str, Decorator style, Elements& out) {
...
std::string CollectText(cmark_node* node) {
  std::string result;
  for (auto* child = cmark_node_first_child(node); child; child = cmark_node_next(child)) {
    ...
    if (type == CMARK_NODE_TEXT || type == CMARK_NODE_CODE) {
      auto* lit = cmark_node_get_literal(child);
      if (lit) result += lit;
  ...
  for (auto* child = cmark_node_first_child(node); child; child = cmark_node_next(child)) {
    switch (cmark_node_get_type(child)) {
      case CMARK_NODE_TEXT: {
        auto* lit = cmark_node_get_literal(child);
        if (lit) SplitWords(std::string(lit), nothing, words);
        break;
      }
      ...
      case CMARK_NODE_CODE: {
        auto* lit = cmark_node_get_literal(child);
        if (lit) words.push_back(text(std::string(lit)) | inverted);
        break;
      }
...
Element RenderInlineWrapped(cmark_node* node) {
...
Element RenderBlock(cmark_node* node, PlainRenderer& renderer);

// Render all children of a node as a vertical stack of blocks.
Elements RenderChildren(cmark_node* node, PlainRenderer& renderer) {
  Elements elems;
  for (auto* child = cmark_node_first_child(node); child; child = cmark_node_next(child)) {
...
Element RenderBlock(cmark_node* node, PlainRenderer& renderer) {
  switch (cmark_node_get_type(node)) {
    ...
    case CMARK_NODE_CODE_BLOCK: {
      auto* fence = cmark_node_get_fence_info(node);
      auto lang = std::string(fence ? fence : "");
      auto* lit = cmark_node_get_literal(node);
      auto code = std::string(lit ? lit : "");
      if (lang == "diff") {
    ...
    case CMARK_NODE_LIST: {
      ...
      for (auto* item = cmark_node_first_child(node); item; item = cmark_node_next(item)) {
...
std::vector<RenderedBlock> PlainRenderer::Render(const std::string& markdown) {
  cmark_node* doc = cmark_parse_document(markdown.c_str(), markdown.size(), CMARK_OPT_DEFAULT);

  std::vector<RenderedBlock> blocks;
  for (auto* node = cmark_node_first_child(doc); node; node = cmark_node_next(node)) {
...
Element PlainRenderer::RenderCodeBlock(const std::string& code, const std::string& /*lang*/) {
  Elements lines;
...
Element PlainRenderer::RenderDiff(const std::string& diff_text) {
  Elements lines;
